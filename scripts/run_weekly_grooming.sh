...
3. python3 scripts/wiki_tool.py root-inbox — triage any loose files at the wiki/ root: promote, move into wiki/inbox/, or delete if debris (empty files can be deleted without asking).
4. Triage wiki/inbox/: read items older than ~2 weeks. For each, per WIKI_SCHEMA.md's Autonomous Promotion Workflow, promote high-confidence durable material into wiki/topics/, wiki/pages/ (type: page, entity, source, or synthesis as appropriate), or wiki/crm/ without asking first. Prefer updating existing canonical pages over creating duplicates, and prefer processing related items in one themed pass over handling each in isolation. Ask for input only before destructive deletes/merges, sensitive CRM claims, major taxonomy changes, or resolving substantive contradictions. Once an item is fully promoted: delete it, or mark it status: archived if a source page's raw_source still needs it for provenance.
5. Check wiki/overview.md's Current State section against reality (entity/synthesis/topic counts, inbox backlog size) and update it if it's gone stale.
6. Re-run the deterministic baseline to confirm everything is still clean.
7. Append one entry to wiki/log.md summarizing what changed: items promoted, items deleted, orphans fixed, lint issues fixed, and anything left pending that needs human judgment.
